...
## Hero visuals on the effect pool (2026-09-25)

Pulling the frame-drop fixes into the hero redesign found that the new
heroes' visuals still made and disposed their own materials: 18 spell
visuals, the Fate and Dusk visuals and every projectile. Releasing those
to the pool throws, and disposing them recompiles their shaders on the
next cast.

- **Four new kinds** cover the looks the pool lacked:
  - `veil`: normal-blended and double-sided, like dust rings and Fate's
    ink;
  - `solid`: smooth lit metal, wood and bone;
  - `fadingSolid`: the same, fading and double-sided, for the whirlwind
    axes and the knell's bell;
  - `fadingRock`: flat-shaded and fading, for dust puffs, orb shards and
    hail clouds.
- **Every projectile disposes itself.** It releases its materials and
  its own geometry. The battle view no longer disposes what a projectile
  drew.
- **Battle effects gained what the hero visuals need.**
  - A plain delay.
  - Delayed projectiles that leave from wherever their caster stands
    when they fly.
  - Ground markers that follow a moving zone.
  - A clear for replay jumps.
  - A bolt with no visual of its own now waits out its chain delay like
    the others. Before, chained plain bolts all flew at once.
- **The warm-up test builds every visual.** It covers all 53 that a
  catalogue id reaches and the four the battle view calls directly.
  - It checks that each one draws a warmed shader and hands its materials
    back undisposed.
  - Its shader key now includes whether a material is opaque, which three
    r186 keys on.
- **Not done.**
  - Hex critters, fate threads and the teleport still make their own
