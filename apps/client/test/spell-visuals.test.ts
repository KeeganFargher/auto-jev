...
import { Mesh, MeshStandardMaterial, Scene, Vector3, type BufferGeometry, type Material, type Object3D } from "three";
import {
  armyOfTheDead,
  avatarJudgment,
  corpseExplosion,
...
test("burning ground and the inferno burst keep their materials for the next cast", () => {
  const particles = createParticleSystem(new Scene());
  const ground = zoneVisual(particles, meteor.id, new Vector3(), 15, 1, 15);
  const burst = formVisual(particles, "inferno", new Vector3(), 12);
  assert.ok(ground !== null && burst !== null);
  const materials = materialsOf([ground.root, burst.root]);
  const disposed = disposals(materials);
