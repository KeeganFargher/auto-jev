...
export interface BattleEffects {
  ...
  projectile(abilityId: string, from: Vector3, aim: () => Vector3, onLand: () => void, delaySeconds?: number, launch?: () => Vector3): number;
  arc(start: Vector3, end: Vector3, abilityId: string, onLand: () => void): void;
  delay(seconds: number, onDone: () => void): void;
...
export const PROJECTILE_SECONDS = 0.16;
...
const SHORTEST_DELAY_SECONDS = 0.001;
...
export function createBattleEffects(scene: Scene, particles: ParticleSystem): BattleEffects {
  ...
  return {
    ...
    projectile(abilityId, from, aim, onLand, delaySeconds = 0, launch = () => from) {
      ...
      let start: Vector3 | null = delaySeconds === 0 ? from : null;

      function flown(progress: number): number | null {
        const elapsed = progress * duration - delaySeconds;
        ...
        return Math.min(1, elapsed / flightSeconds);
      }
      ...
      if (visual !== null) {
        run(
          ...
          duration,
          (progress) => {
            const along = flown(progress);

            for (const object of visual.objects) {
              object.visible = along !== null;
            }

            visual.place(start ?? launch(), aim(), along ?? 0);
          },
          onLand,
          () => visual.dispose(),
        );
      ...
      run(
        ...
        (progress) => {
          const along = flown(progress);
          const end = aim();
          bolt.visible = along !== null;
          bolt.position.lerpVectors(start ?? launch(), end, along ?? 0);
          bolt.lookAt(end);
    ...
    delay(seconds, onDone) {
      run([], Math.max(SHORTEST_DELAY_SECONDS, seconds), () => {}, onDone, () => {});
    },
