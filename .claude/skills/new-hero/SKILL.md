...
- Take effect materials from the pool (`effectMaterials.<kind>.take()` in `game/views/effect-materials.ts`) and hand them back with `releaseEffectMaterial` in `dispose`; never `new` a material per cast or `dispose()` one. A disposed material deletes its shader, and the next cast stalls a frame compiling it. A new look that no kind covers is a new kind in the pool. Share geometry across casts, and give mesh geometry normals (the warm-up's shaders assume them); the warm-up test in `test/battle-effects.test.ts` builds every visual a catalogue id reaches, so add one only if the battle view calls it directly.
